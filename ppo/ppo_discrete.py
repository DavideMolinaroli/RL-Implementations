...
class PPOMemory:
    ...
    def generate_batches(self):
        n_states = len(self.states) # equal to N 
...
class ActorNetwork(nn.Module):
    ...
    def forward(self, state):
        ...
        dist = Categorical(dist) # turn dist into a distribution from which we can sample actions
        return dist
...
class Agent:
    ...
    def learn(self):
        for _ in range(self.n_epochs):
            ...
            # batches are not shuffled here
            values = vals_arr
            advantage = np.zeros(len(reward_arr), dtype=np.float32)
            # these lengths are equal to N
            # iterate through last N rewards. They are not shuffled yet, so they come from subsequent transitions
            for t in range(len(reward_arr)-1):
                discount = 1
                a_t = 0
                for k in range(t, len(reward_arr)-1): # equations 11 and 12 of PPO paper
                    # if at step k (within the current rollout buffer) the episode is finished, then the next state is the initial one of the next episode.
            ...
            for batch in batches:
                ...
                # states contains multiple observations,
                # hence dist is a list of distributions, critic_value is a list of values
